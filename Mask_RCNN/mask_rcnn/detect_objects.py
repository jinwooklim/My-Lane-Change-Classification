...
class DetectObjects:
    ...
    def save_masked_images(self):
        ...
        for filename in filenames:
            print(filename)
            if not filename.startswith('.'):
                img = skimage.io.imread(os.path.join(IMAGE_DIR, filename))
                results = model.detect([img], verbose=0)
                # Visualize results
                r = results[0]
                r['masks'], r['rois'], r['class_ids'], r['scores'] = self.filter_masks(r['masks'], r['rois'], r['class_ids'], r['scores'])
                visualize.display_instances(img, r['rois'], r['masks'], r['class_ids'], class_names, r['scores'], save_option=1, save_path=os.path.join(OUTPUT_DIR, filename))
                plt.close('all')
    ...
    def filter_masks(masks, rois, class_ids, scores):
        # cyan 0,255,255
        # print(rois.shape) # (6, 4)
        N = rois.shape[0] # 6
        if N == 0:
            return masks, rois, class_ids, scores
        cropped_regions = np.array([[650, 874, 0, 1164],
                                    [350, 450, 482, 682],
                                    [0, 100, 0, 1164]])
        indices = np.ones(N, dtype=bool)
        for i in range(N):
